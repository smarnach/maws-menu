...
impl History {
    ...
    pub fn account_index(&self, account: &str) -> Option<usize> {
        self.config.last_accounts.iter().position(|x| x == account)
    }

    pub fn default_account(&self) -> Option<String> {
        self.config.last_accounts.first().map(ToOwned::to_owned)
    }

    pub fn default_role(&self, account: &str) -> Option<String> {
        self.config.roles.get(account).map(ToOwned::to_owned)
    }
